...
# File       [ evaluator.sh ]
# Author     [ FrankChiang ]
# Synopsis   [ To evaluate the score with pd24 ]
...
# check files
BASEDIR=$(dirname "$0")
FILE_LIST=("$BASEDIR/pd24_results.list")
for FILE_NAME in ${FILE_LIST[@]}; do
    if [ ! -f $FILE_NAME ]; then
        echo "[Eval] error: Please check file '$FILE_NAME' exist."
        echo "(ps. pd24_result.list should be in the same dir with evaluator.sh)"
        exit 1
    fi
done
...
for idx in ${!HPWL_LIST[@]}; do
    if (( $(echo "${HPWL_LIST[$idx]} >= $HPWL" |bc -l) )); then
        if [ $idx -eq 0 ]; then
            ...
            QUALITY_SCORE=`echo $SCORE1 + $Y_RATIO | sed 's/e/*10^/g' | bc`
        ...
        X_RATIO=`awk "BEGIN {print $X_DIFF / $X_TOTAL_DIFF}"`
        Y_RATIO=`awk "BEGIN {print $X_RATIO * $Y_DIFF}"`
        QUALITY_SCORE=`echo $SCORE2 + $Y_RATIO | sed 's/e/*10^/g' | bc`
